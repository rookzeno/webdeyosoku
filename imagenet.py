...
class imagenet():
    def __init__(self,img):
        ...
        img = image.img_to_array(img).reshape(1,64,64,3)
        self.img = img
    def load_image(filename):
        img = image.load_img(filename, target_size=(64, 64))
        return image.img_to_array(img)
    def deep(self):
        model = MobileNet(input_shape=(64,64,3), alpha=1.0, depth_multiplier=1, dropout=1e-3, include_top=True, weights=None, input_tensor=None, pooling=None, classes=1000)
